...
namespace EasyLife.PageModels.Edit_Item_With_Order_Submethods
{
    public class Edit_Version3
    {
        public static async Task<string> Edit_Version3_Methode(Transaktion Virtuelle_Transaktion , Transaktion Transaktion, Auftrag Virtueller_Auftrag, List<Transaktion> transaktion_list, string Betrag, string Zweck, string Notiz, DateTime Datum, int OrderId, bool Revive_Switch, int Full_Order, string Anzahl_an_Wiederholungen)
        {
            ...
            Auftrag last_order = new Auftrag()
            {
                ...
                Option = Transaktion.Auftrags_Option,
                Id = OrderId
            };

            int compare = 0;

            if (virtueller_auftrag.Id != last_order.Id)
            {
                compare = 1;
            }
            if (virtueller_auftrag.Anzahl_an_Wiederholungen != last_order.Anzahl_an_Wiederholungen)
            {
                compare = 1;
            }
            if (virtueller_auftrag.Art_an_Wiederholungen != last_order.Art_an_Wiederholungen)
            {
                compare = 1;
            }
            if (virtueller_auftrag.Speziell != last_order.Speziell)
            {
                compare = 1;
            }
            if (virtueller_auftrag.Option != last_order.Option)
            {
                compare = 1;
            }
            ...
            if (compare != 0)
            {
                ...
                int capacity = transaktion_list2.Count();
                ...
                if (list_of_kinds.Contains(Virtuelle_Transaktion.Art_an_Wiederholungen) == true)
                {
                    if (Count < capacity)
                    {
                        ...
                        transaktions.AddRange(transaktion_list2.GetRange(Count, capacity - Count));
                    ...
                    if (Count > capacity)
                    {
                        List<Transaktion> wiedrholungen = Enumerable.Repeat(Virtuelle_Transaktion, Count - capacity).ToList();
                ...
                if (Virtuelle_Transaktion.Art_an_Wiederholungen == "Spezielle Tage")
                {
                    ...
                    if (Count > capacity)
                    {
                        List<Transaktion> wiedrholungen = Enumerable.Repeat(Transaktion, Count - capacity).ToList();
